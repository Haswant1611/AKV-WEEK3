...
class CronTaskService {
  constructor() {
    // Initialize cron job for every 15 mins'
  ...
  async processUploadedFiles() {
    const pending_files = await knex('file_uploads')
  ...
  setSocketIo(io) {
    this.io = io;
    console.log('Socket.io instance set for CronTaskService');
  ...
  async processFile(fileUpload) {
    try {
      ...
      if(this.io){
        this.io.emit('file-processing', {
          fileId: fileUpload.id,
          fileName: fileUpload.file_name,
          status: 'processing',
          message:`Started Processing file ${fileUpload.file_name}`
        }); 
      }
      ...
      for (let i = 0; i < totalRecords; i++) {
        const result = await this.processRecord(data[i], fileUpload.id);
        ...
        // console.log(per_records_processed+'% records processed');
        if (this.io) {
      ...
      if(this.io){
        this.io.emit('fileProgress', {
          ...
          fileName: fileUpload.file_name,
          progress: 100,
          message: `Processing completed. ${successfulRecords.length} records processed`
        });
      }
  ...
  async processRecord(record, userId) {
    ...
    try {
      ...
      const product_image = record.product_image || record['Product Image'];
      const category = record.category || record['Category'];
      const vendors = record.vendors || record['Vendors'];
      const quantity_in_stock = record.quantity_in_stock || record['Quantity'];
      const unit_price = record.unit_price || record['Unit Price'];

      // Collect validation errors
      if (!productName) errors.push('Product name is required');
      if (!category) errors.push('Category is required');
      if (!vendors) errors.push('Vendors are required');
      if (quantity_in_stock === undefined || isNaN(quantity_in_stock)||quantity_in_stock<0) 
      {
            errors.push('Quantity is required and must be a valid positive number');
          }
          if (!unit_price && unit_price<0) {
            errors.push('Unit Price is required and should be valid positive number');
          }
  ...
  async generateErrorSheet(invalidRecords, fileId) {
    try 
    {
      ...
      const errorFileKey = `haswant/error-sheets/${fileId}_errors.xlsx`;
...
module.exports = new CronTaskService(); 