...
const { use } = require('./inventory.routes');
...
const generatePresignedUrl = async (req, res) => {
  try {
    ...
    const params = {
      ...
      Key: `haswant/products/${userId}/${Date.now()}_${fileName}`,
...
const generatePresignedUrlProductImage = async (req, res) => {
  try {
    const { fileName, fileType } = req.body;
    const userId = req.user.userId;
    const key = `haswant/products/${userId}/${Date.now()}_${fileName}`;
...
const addProduct = async (req, res) => {
  const { productName, product_image, category, vendors, quantity, status , unit_price} = req.body;
      ...
      if (columns.length > 0) {
          ...
          columns.forEach(column => {
            switch(column) {
              ...
              case 'quantity_in_stock':
                ...
                break;
              case 'unit_price':
                this.orWhere('products.unit_price', 'like', `%${search}%`);
                break;
            }
          });
        });
        ...
        query = query.andWhere(function() {
          this.where('products.product_name', 'like', `%${search}%`)
            ...
            .orWhere('products.unit_price', 'like', `%${search}%`)
            .orWhere(function() {
              const statusSearch = search.toLowerCase();
              if (statusSearch.includes('available')) {
                this.orWhere('products.status', '=', 1);
...
const updateProduct = async (req, res) => {
  const { productId } = req.params;
  const { product_name, category, vendors, quantity_in_stock, status, product_image , unit_price} = req.body;
  const trx = await knex.transaction();
...
const uploadFileForProcessing = async (req, res) => {
  try {
    const { fileName, fileType } = req.body;
    const userId = req.user.userId;
    const fileKey = `haswant/uploads/${userId}/${Date.now()}_${fileName}`;
...
const triggerProcessing = async (req, res) => {
  try {
    await cronService.processUploadedFiles();
